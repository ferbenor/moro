...
{
    public partial class ClientesLinq : Form, IAccesoMetodos
    {
        ...
        BindingListView<ModeloDB.cliente> lsPrincipal = null;

        private object objetoLocal = new object();
        ...
        private void CargarCampos()
        {
            ...
            this._lblEstado.Text = ((ModeloDB.cliente)this.objetoLocal).fkpersona.fkestadospersona.descripcion;
            this._lblEstado.ForeColor = ((ModeloDB.cliente)this.objetoLocal).fkpersona.idestadopersona == 2 ? System.Drawing.Color.Red : System.Drawing.Color.Blue;
        ...
        private void CargarCombos()
        {
            ...
            General.CargarCombos(this._cboTipoPersona, "Objeto", "Objeto", ClientePr.Instancia.ListaTipoPersona);
            General.CargarCombos(this._cboTipoDocumento, "Objeto", "Objeto", ClientePr.Instancia.ListaTipoDocumento);
            General.CargarCombos(this._cboGenero, "Objeto", "Objeto", ClientePr.Instancia.ListaGenero);
            General.CargarCombos(this._cboIdEstadoCivil, "id", "Objeto", ClientePr.Instancia.ListaEstadoCivil);
        ...
        private void SeleccionFila(int unaColumna, int unId)
        {
            ...
            foreach (DataGridViewRow registro in this.dgrLista.Rows)
                if (registro.Cells["colId"].Value.ToString().Equals(unId.ToString()))
                {
                    fila = registro.Index;
                    break;
                }
        ...
        public void Nuevo()
        {
            ...
            this.objetoLocal = new ModeloDB.cliente();
            ...
            {
                {
                    ...
                    try
                    {
                        ClientePr.Instancia.Grabar((ModeloDB.cliente)this.objetoLocal);
                        this.idRegistro = ((ModeloDB.cliente)this.objetoLocal).idpersona;
        ...
        public void Eliminar()
        {
            if (this._txtIdRO.Text != "0")
                ...
                {
                    this.objetoLocal = new cliente() { idpersona = (int)ValorCelda("colId") };
                    ClientePr.Instancia.Borrar((ModeloDB.cliente)this.objetoLocal);
        ...
        public void Actualizar()
        {
            try
            {
                ...
                if (estadoBarra != EstadoBarraEnum.EDITANDO)
                    idRegistro = (int)ValorCelda("colId");
                int columna = 1;
                ...
                //    lsPrincipal = new BindingListView<ModeloDB.cliente>(ClientePr.Instancia.Registros("fkpersona.identificacion.Contains(@0)", this.txtBusqueda.Text));
                ////else
                //    lsPrincipal = new BindingListView<ModeloDB.cliente>(ClientePr.Instancia.Registros("idpersona == -1"));
                ClientePr.Instancia.Registros(this.expresion.ToString(), this.parametros.ToArray()).CargarGrid(this.dgrLista);
                //this.dgrLista.DataSource = lsPrincipal;
                //{
        ...
        {
            this._txtFKConyugeRO.Tag = 1;
            this._txtBarrioRO.Tag = 3;
            this._txtFKProfesioneRO.Tag = 4;

            this._bbqConyugue.Tag = this._txtFKConyugeRO;
            this._bbqProfesion.Tag = this._txtFKProfesioneRO;
        ...
        private void GestionBarra(EstadoBarraEnum unEstado)
        {
            ...
            switch (unEstado)
            {
                ...
                case EstadoBarraEnum.EDITANDO:
                    if (((ModeloDB.cliente)objetoLocal).idpersona != 0)
                        this.editando = true;
                    this.SuspendLayout();
                    if (this.tabLista.Visible)
                    {
        ...
        {
            if (this._cboIdEstadoCivil.SelectedIndex == 1)
                this._txtFKConyugeRO.Focus();
            //else
        ...
        private void btnAceptarB_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(this.txtBusqueda.Text))
            {
                this.expresion.Append("fkpersona.identificacion.Contains(@0) || (fkpersona.apellido + \" \" + fkpersona.nombre).Contains(@0)");
                this.parametros.Add(this.txtBusqueda.Text);
            }
            this.Actualizar();
            //GestionBarra(EstadoBarraEnum.NINGUNO);
        }
        ...
        private void verificarRegistro(string unCodigo)
        {
            ...
            //CargarCombos();
            object objeto = ClientePr.Instancia.RegistroPorIdentificacion(unCodigo);
            if (objeto != null)
            {
        ...
        {
            this.lsPrincipal.ApplyFilter(
                delegate(cliente item)
                {
                    // uses ToLower() to ignore case of text.
                    return item.fkpersona.nombre.ToLower().Contains(this.txtFiltro.Text.ToLower()) || item.fkpersona.apellido.ToLower().Contains(this.txtFiltro.Text.ToLower())
                        || item.fkpersona.identificacion.Contains(this.txtFiltro.Text);
                }
            ); ;
        }
    }
