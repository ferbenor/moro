...
namespace MotorInformixIds
{
    class IfxDAO : Datos.ProveedorDatos
    {
        #region PROPIEDADES
        string mensaje;
        private IfxConnection conexionLocal;
        bool exeAutoNum = false;
        IfxCommand comandoPreparado;
        
        public IfxConnection Conexion
        {
            get
            {
                if (conexionLocal == null)
                {
                    conexionLocal = new IfxConnection(GlobalConnectionString);
                    if (conexionLocal == null)
        ...
        {
            IfxConnection conn = this.Conexion;
        ...
        {
            if (unaTransaccion != null)
                ((IfxTransaction)unaTransaccion).Commit();
        }

        public override void RollBack(object unaTransaccion)
        {
            if (unaTransaccion != null)
                ((IfxTransaction)unaTransaccion).Rollback();
        }
        ...
        public override void Close(object unaConexion)
        {
            if (unaConexion != null)
            {
                ((IfxConnection)unaConexion).Close();
                ((IfxConnection)unaConexion).Dispose();
                unaConexion = null;
            }
        }
