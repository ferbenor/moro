...
{
    /// <summary>
    /// Reflection based, Generic Comparer class for in-line or instance based Sorting
    /// </summary>
    /// <example>
    ...
    /// or
    /// 
    /// GenericComparer genericComparer = new GenericComparer<DataObject>();
    /// genericComparer.MemberName = "ID";
    /// genericComparer.SortOrder = "asc";
    /// 
    /// array.sort(genericComparer);
    /// </example>
    public class ComparadorGenerico<T> : IComparer<T>
    {
        #region Fields
        ...
        public ComparadorGenerico(string memberName, System.Windows.Forms.SortOrder sortOrder, List<object> methodParameters)
        {
            ...
            if (mi.Length > 0)
            {
                ...
                else if (mi[0].MemberType == MemberTypes.Method)
                {
                    ...
                    if (methodParameters != null && methodParameters.Count > 0)
                    {
                        ...
                        methodInfo = thisUnderlyingtype.GetMethod(memberName, signatureTypes);
                    }
                    else
                    {
                        methodInfo = thisUnderlyingtype.GetMethod(memberName, signatureTypes);
                ...
                {
                    throw new Exception("Member name: " + memberName + " is not a Public Property or a Public Method in Type: " + thisUnderlyingtype.Name + ".");
                }
            }
            else
            {
        ...
        private IComparable GetComparable(T obj)
        {
            if (methodInfo != null)
            {
                return (IComparable)methodInfo.Invoke(obj, methodParameters.ToArray());
            }
            else
            {
                object objetoValue;
                if (propertyInfo.PropertyType == typeof(System.Drawing.Image))
                    objetoValue = "";
                else
                    objetoValue = (IComparable)propertyInfo.GetValue(obj, null);
                if (objetoValue == null)
                    objetoValue = "";
                return (IComparable)objetoValue;
            }
